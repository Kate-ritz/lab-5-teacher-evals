...
evals <- read_csv("data-raw/teacher_evals.csv", 
                  col_types = list(
                    teacher_id = "c",
                    question_no = "c"
                  ))
...
> These data represent student evaluations of teaching on a Likert-scale. Each case is the average score of each question for each course, accompanied by information on the class, such as grades, number of students, teacher, etc. 
...
> Each observation is a question on the evaluation for each course.
...
teacher_evals_clean <- evals |> 
  rename(sex = gender) |> 
  mutate(resp_count = no_participants*resp_share) |> 
  filter(resp_count > 10 | is.na(resp_count)) |> 
...
# variable types specified when reading in original data
...
# code chunk for Q6
distinct(teacher_evals_clean, course_id)
distinct(teacher_evals_clean, teacher_id)
```

> There are 71 unique teachers, and 91 unique courses.
...
colSums(is.na(teacher_evals_clean))
teacher_evals_clean |> 
  filter(is.na(no_participants) | is.na(resp_share) | is.na(percent_failed_cur))
```
> All instructors of course 0000-SEM-NP-b have missing values in the variables no_participants, resp_share, and percent_failed_cur.
...
# code chunk for Q8
tally(~academic_degree, teacher_evals_clean)
tally(~sex, teacher_evals_clean)
tally(sex~academic_degree, teacher_evals_clean)
df_stats(~seniority, teacher_evals_clean)
df_stats(seniority~sex, teacher_evals_clean)

```

> There are 303 female teachers and 359 male professors. Of the female professors, 51.5% hold doctorate degrees, 44.2% hold master's degrees, and 4.3% hold no degree. Of the male teachers, 56.0% hold doctorate degrees, 25.6% hold master's degrees, 12.8% hold no degree, and 5.0% are professors (or professionals?). The mean seniority overall is 5.28 years. For female teachers, the mean seniority is 4.90 years, and for males it is 5.61 years.
...
# code chunk for Q9
question_count <- teacher_evals_clean |> 
...
> Only three teacher-course combinations asked all nine questions.
...
teacher_evals_clean |> 
  ...
  group_by(teacher_id) |>
  summarize(mean(SET_score_avg)) |> 
  arrange(`mean(SET_score_avg)`)

```

> Course teacher 3521 was the worst rated, and there were six teachers tied for the best rated for question 1.
...
teacher_evals_clean |> 
  filter(seniority == 1) |>
  group_by(teacher_id) |>
  summarize(mean(percent_failed_cur)) |> 
  arrange(`mean(percent_failed_cur)`)

```

> Teacher 101703 had the highest average percentage of students failing, while teacher 103092 had the lowest. 
...
teacher_evals_clean |> 
  filter(sex == "female") |>
  filter(academic_degree == "dr" | academic_degree == "ma") |> 
  group_by(teacher_id) |>
  summarize(mean(resp_share)) |> 
  arrange(`mean(resp_share)`)
```

> Teacher 76390 had the highest average percentage of students responding to the questions, while teacher 40758 had the lowest. 
